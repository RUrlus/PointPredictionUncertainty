...
def plot_dense_binary_scatter(
    ...
    lh = LegendHandler(ax)
    mask = y.astype(bool)
    ax.scatter(X[~mask, 0], X[~mask, 1], marker=".", s=30, c="C0", label="$y=0$", alpha=alpha, zorder=zorder, **kwargs)
    ax.scatter(X[mask, 0], X[mask, 1], marker="x", s=15, c="C1", label="$y=1$", alpha=alpha, zorder=zorder, **kwargs)
