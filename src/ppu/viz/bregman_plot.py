...
def draw_classifier(generator, n_samples, names, classifiers, models, rescale=False):
    ...
    for name, clf in zip(names, classifiers):
        ...
        response = _check_boundary_response_method(clf, "auto")(
            X_grid
        ...
        ax.set_title(name)
        ax.text(
            x_max - 0.3, y_min + 0.3, ("%.2f" % score).lstrip("0"), horizontalalignment="right"
...
def draw_classifiers_BIbs(generator, n_samples, selected_clfs, selected_models, n_models=64, rescale=False):
    ...
    for name in selected_models:
        with threadpool_limits(limits=1):
            response = get_BI(
                X_grid, bootstrap_models(selected_clfs[name], generator, reps=n_models, n_samples=n_samples)
            )
        response = response.reshape(xs.shape)
        ax = axs[1][i]
    ...
    plt.subplots_adjust(bottom=0.2, top=0.9, wspace=0.1, hspace=0.3)
...
def draw_diff_variances(classifier, generator, n_samples, variances, n_models=64):
    ...
    for i in range(n_fig):
        ...
        with threadpool_limits(limits=1):
            sep_model = get_models(
                classifier,
                generator,
                reps=n_models,
                n_samples=n_samples,
                scale=(variances[i], variances[i]),
                class_sep=1.7,
            )
...
def draw_diff_class_sep(classifier, generator, n_samples, seps, n_models=64):
    ...
    for i in range(n_fig):
        ...
        with threadpool_limits(limits=4):
            response2 = get_BI(
                X_grid, bootstrap_models(classifier, generator, reps=n_models, n_samples=n_samples, class_sep=seps[i])
            )
        vmin = min(vmin, *response2)
        vmax = max(vmax, *response2)
        response2 = response2.reshape(xs.shape)
...
def draw_diff_bootstrap(classifier, generator, n_samples, bootstrap, n_models=64):
    ...
    for i in range(num):
        with threadpool_limits(limits=4):
            response2[i] = get_BI(
                X_grid, bootstrap_models(classifier, generator, reps=bootstrap[i], n_samples=n_samples)
            )
