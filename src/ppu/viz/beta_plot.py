...
def draw_beta_bs(classifier, generator, n_samples, bootstrap, n_models=64, threshold=0.5):
    ...
    for i in range(num):
        with threadpool_limits(limits=4):
            response2[i] = get_Beta(
                X_grid, bootstrap_models(classifier, generator, reps=bootstrap[i], n_samples=n_samples), threshold
            )
...
def draw_beta_pdf(start, end, classifier, generator, n_samples, n_models=64, threshold=0.5, n_pdf=50):
    ...
    surface = go.Surface(x=x_vals, y=y_vals, z=z_vals, colorscale="Viridis")

    fig.add_trace(surface)

    fig.update_layout(scene={"xaxis_title": "Line Segment Points", "yaxis_title": "y", "zaxis_title": "PDF"})
